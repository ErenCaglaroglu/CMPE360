...
//Collect shape information into a neat package
var shapes = {
...
// Load data into points and colors arrays - runs once as the page loads.
var points = [];
var colors = [];
...
function render() {
    ...
    mv = mult(translate(0, 0, -15), mv);

    gl.uniformMatrix4fv(mvLoc, gl.TRUE, flatten(transpose(mv)));
    gl.drawArrays(shapes.axes.type, shapes.axes.start, shapes.axes.size);

    // Step 5: Draw a wireCube centered (0,0,0) relative to the axes.
    gl.drawArrays(shapes.wireCube.type, shapes.wireCube.start, shapes.wireCube.size);

    // Step 6: Move this cube so that it is centered at (1,0,0) relative to the axes.
    mv = mult(translate(1, 0, 0), mv);
    gl.drawArrays(shapes.wireCube.type, shapes.wireCube.start, shapes.wireCube.size);

    // Step 7: Draw a second cube in a new color (blue) and rotate it 45 degrees around the y-axis.
    mv = mult(rotate(45, [0, 1, 0]), mv);
    loadShape(shapes.wireCube, gl.LINE_STRIP);
    gl.drawArrays(shapes.wireCube.type, shapes.wireCube.start, shapes.wireCube.size);

    // Step 8: Place this rotated cube directly above the first cube.
    mv = mult(translate(1, 2, 0), mv);
    gl.drawArrays(shapes.wireCube.type, shapes.wireCube.start, shapes.wireCube.size);

    // Step 9: Use orthographic projection instead of perspective.
    p = ortho(-2, 2, -2, 2, 0.1, 100.0);
    gl.uniformMatrix4fv(projLoc, gl.FALSE, flatten(transpose(p)));

    // Step 10: Rotate everything to look down at the top of the boxes and see the blue z-axis.
    mv = mult(rotate(90, [1, 0, 0]), mv);

    // Step 11: Rotate everything to see all three axes along with the cubes.
    mv = mult(rotate(45, [1, 1, 1]), mv);
