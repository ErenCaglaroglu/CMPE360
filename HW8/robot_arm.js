...
var shoulder = 0, elbow = 0, thumb = 0, finger = 0;
...
function render() {
    ...
    mv = mult(mv, translate(1.0, 0.0, 0.0));
    // Rotate Finger 1 relative to the elbow
    mv = mult(mv, rotate(-thumb, vec3(0, 0, 1)));
    // Position Finger 1 segment
    mv = mult(mv, translate(0.5, 0.25, 0.0));
    // Scale and Draw thumb segment
    matStack.push(mv);
    ...
    // Position Finger 2 Joint
    mv = mult(mv, translate(1.4, -0.3, -0.3));
    // Rotate Finger 2 relative to the elbow
    ...
    // Position Finger 3 Joint
    mv = mult(mv, translate(1.0, 0.0, 0.0));
    // Position Finger 3 segment
    mv = mult(mv, translate(-1, 0, 0.5));
    // Scale and Draw Finger 3 segment
    matStack.push(mv);
    mv = mult(mv, scale(1.0, 0.2, 0.5));
    gl.uniformMatrix4fv(mvLoc, gl.FALSE, flatten(transpose(mv)));
    gl.drawArrays(armShape.type, armShape.start, armShape.size);
    ...
    // Position Finger 4 Joint
    mv = mult(mv, translate(1.0, 0.0, 0.0));

    // Position Finger 4 segment
    mv = mult(mv, translate(-1, 0, 0.5));
    // Scale and Draw Finger  4 segment
    matStack.push(mv);
    mv = mult(mv, scale(1.0, 0.2, 0.5));
    gl.uniformMatrix4fv(mvLoc, gl.FALSE, flatten(transpose(mv)));
    gl.drawArrays(armShape.type, armShape.start, armShape.size);
...
function handleKeys(timePassed) {
    ...
    // Finger1 update (baş parmak)
    if (shift && isPressed("F")) {
        if (thumb < 0) thumb = (thumb + d); // Move Finger1 (baş parmak) upward
        else thumb = 0;
    }
    if (!shift && isPressed("F")) {
        if (thumb > -15) thumb = (thumb - d); // Move Finger1 (baş parmak) downward
        else thumb = -15;
    }
